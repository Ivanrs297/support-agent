...
#
# Afterwards, fill in the .env it creates — the deploy will fail loudly until
# GROQ_API_KEY is set, which is the intended behaviour.
#
# COMPOSE_DIR is overridable, and matches remote-deploy.sh. In this repository
# the deployed system lives under solution/, so production is set up with:
#
...
Next:
  1. Put a real GROQ_API_KEY in $COMPOSE_DIR/.env
